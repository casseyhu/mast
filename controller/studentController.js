...
exports.create = (req, res) => {
  ...
  if (emptyFields(student)) {
    let err_msg =
      "Error in adding student. Please check that all necessary student information are filled out.";
  ...
  let query = Degree.findOne({
    where: {
      ...
      requirementVersion: Number(
        student.degreeYear + semDict[student.degreeSem]
      ),
    },
  })
    .then((degree) => {
      ...
      if (grad_date < entry_date) {
        res
          .status(500)
          .send("Graduation date cannot be earlier than entry date.");
        return;
      }
      ...
      Student.create({
        ...
        entrySemYear: Number(
          student.entryYear.concat(semDict[student.entrySem])
        ),
        ...
        .catch((err) => {
          console.log(err);
          err_msg =
            "Error in adding student. Please check student information type (i.e. SBUID must be numbers 0-9).";
          if (
            err.parent.code !== undefined &&
...
exports.upload = (req, res) => {
  ...
  form.parse(req).on("file", (field, file) => {
    if (file.type !== "text/csv" && file.type !== "application/vnd.ms-excel")
      ...
      if (isValid) res.status(200).send("Success");
    }
  });
};
...
exports.findById = (req, res) => {
  Student.findById(req.params.sbuId)
    .then((student) => {
      res.send(student);
    })
    .catch((err) => {
...
exports.delete = (req, res) => {
  Student.destroy({ where: { sbuId: req.params.sbuId } })
    .then(() => {
...
async function uploadStudents(csv_file) {
  ...
  for (let i = 0; i < csv_file.data.length - 1; i++) {
    ...
    let semYear = Number(
      student_info.entry_year + sems_dict[student_info.entry_semester]
    );
    let graduated =
      Number(
        student_info.graduation_year +
          sems_dict[student_info.graduation_semester]
      ) <= currentGradYear
        ? 1
        : 0;
    ...
    else await Student.create(values);
  }
